...
fov = 140 # Field of view [mm]
spiral_res = 1.71 # Resolution [mm]
...
recon_id = "60325_scans"# For multi-slice runs: "250320_mult_slice" # For offset runs: "DIFF_OFFSETS_IV_SC"*spiral "IV_SC_140mm_1p71_R1" 

# Dictionary to map spirals to crop indices for crop of spiral trajectory
...
params_general[:recon_save_path] = joinpath(params_general[:project_path],"Reconstructions", params_general[:recon_id], "DiffOffsets_$(fov)_R$R") #"all_measurements_$(fov)R$R") # "DiffOffsets_$(fov)_R$R" #)  # "No_SENSE"
...
params_general[:map_scan_filename] = "Fieldmaps/inVivo_fieldmap_prescan.h5"#"inVivo_fieldmap_prescan.h5"# ms_MID00036_fm_2mm_magph.h5"#"Fieldmaps/lTE_ms_MID00053_fm_2mm_magph.h5"# 250320 first spirals "Fieldmaps/ms_MID00036_fm_2mm_magph.h5" # 60325 scans :inVivo_fieldmap_prescan.h5 #"Fieldmaps/32ch_phantom_fm_converted.h5"# # Cartesian dual-echo file, for coil and B₀ maps calculation [Input]
params_general[:map_scan_filename_stem] = "inVivo_fieldmap_prescan.h5" #"32ch_phantom_fm_converted.h5" #
...
#Output files
params_general[:scan_filename_stem] = "$current_spiral" #"32ch_140mm1p71mmR1.h5"# # Main file name when saving the result
...
params_general[:b0_map_save_filename] = string(params_general[:b0_map_beta])*"_b0map.nii"; splitext(params_general[:map_scan_filename_stem])[1] * "_"* string(params_general[:b0_map_beta])*"_b0map.nii"; # File name for calculated off-resonance (B₀) maps [Output]
...
if (params_general[:do_correct_with_b0_map])
    ...
    params_general[:recon_save_filename] = params_general[:scan_filename_stem]*"_nob0"
end


# otherwise, just concat _recon.nii to file name
params_general[:recon_save_filename] = params_general[:recon_save_filename] * "_recon_2.nii" #noSENSE_
...
params_general[:scan_fullpath] = joinpath.(params_general[:spiral_path], "ss_"*params_general[:scan_filename_stem]*".h5") # Full paths of raw k-space data files of spiral acquisition. "ms_"* for multi-slice

# Endret fra params_general[:results_path] til params_general[:recon_save_path]
params_general[:processed_map_scan_fullpath] = joinpath(params_general[:results_path], params_general[:processed_map_scan_filename]) # Full paths of pre-processed Cartesian dual-echo data [Output]
...
params_general[:map_save_fullpath] = joinpath(params_general[:results_path], params_general[:map_save_filename]) # Full paths of reconstructed dual-echo Cartesian images [Output]
